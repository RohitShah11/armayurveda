...
@push('styles')
<style>
.category-card{display:block;background:#fff;border:1px solid #eee;border-radius:16px;overflow:hidden;height:100%;color:#212529;text-decoration:none;box-shadow:0 7px 22px rgba(0,0,0,.06);transition:.2s}.category-card:hover{color:#7b1e3a;transform:translateY(-4px);box-shadow:0 12px 30px rgba(0,0,0,.1)}
.category-image{height:210px;width:100%;object-fit:cover;background:#f3f4f6}.category-placeholder{height:210px;display:flex;align-items:center;justify-content:center;background:#f3f4f6;color:#9ca3af;font-size:44px}.category-body{padding:20px}.subcategory-link{display:inline-block;margin:4px 3px 0 0;padding:5px 10px;border-radius:18px;background:#f6e9ee;color:#7b1e3a;text-decoration:none;font-size:12px}.subcategory-link:hover{background:#7b1e3a;color:#fff}
</style>
@endpush
@section('content')
    ...
    @forelse($categories as $category)
      <div class="col-sm-6 col-xl-3"><div class="category-card">
        <a href="{{ route('catalog.category', $category) }}" class="text-decoration-none text-dark d-block">
          @if($category->image)<img class="category-image" src="{{ Storage::url($category->image) }}" alt="{{ $category->name }}">@else<div class="category-placeholder"><i class="fa fa-layer-group"></i></div>@endif
        </a>

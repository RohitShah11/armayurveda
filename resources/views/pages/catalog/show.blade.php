...
@push('styles')
<style>.customer-panel{background:#fff;border-radius:14px;padding:22px;box-shadow:0 7px 22px rgba(0,0,0,.06)}.detail-image{width:100%;max-height:470px;object-fit:contain;background:#f8f9fa;border-radius:16px}.gallery-thumb{width:80px;height:80px;object-fit:cover;border:1px solid #ddd;border-radius:10px}.detail-price{font-size:30px;font-weight:900;color:#7b1e3a}.detail-section{border-top:1px solid #eee;padding-top:20px;margin-top:20px}</style>
@endpush
@section('content')
<div class="container-fluid py-4">
  ...
  <div class="customer-panel mt-3"><div class="row g-4">
    <div class="col-lg-5">@if($product->image)<img id="mainProductImage" class="detail-image" src="{{ Storage::url($product->image) }}" alt="{{ $product->name }}">@else<div class="detail-image d-flex align-items-center justify-content-center text-muted" style="height:400px"><i class="fa fa-box fa-4x"></i></div>@endif
      @if($product->gallery_images)<div class="d-flex flex-wrap gap-2 mt-3">@foreach($product->gallery_images as $gallery)<img class="gallery-thumb" role="button" src="{{ Storage::url($gallery) }}" alt="" onclick="document.getElementById('mainProductImage').src=this.src">@endforeach</div>@endif
    </div>
  ...
  @if($relatedProducts->isNotEmpty())<h4 class="fw-bold mt-5 mb-3">Related Products</h4><div class="row g-3">@foreach($relatedProducts as $related)<div class="col-sm-6 col-lg-3"><a class="text-decoration-none text-dark" href="{{ route('catalog.show',$related) }}"><div class="customer-panel h-100">@if($related->image)<img class="w-100 rounded mb-3" style="height:150px;object-fit:cover" src="{{ Storage::url($related->image) }}" alt="">@endif<strong>{{ $related->name }}</strong><div class="text-success mt-2">₹{{ number_format($related->retail_price,2) }}</div></div></a></div>@endforeach</div>@endif
</div>
@endsection

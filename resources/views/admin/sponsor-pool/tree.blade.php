...
@section('title', 'Sponsor Pool Tree')
@section('page-title', 'Sponsor Pool Tree View')
...
<div class="tree-toolbar mb-4">
  <div>
    <h4 class="fw-bold mb-1">Sponsor Autopool Tree</h4>
    <p class="text-muted mb-0">Admin root with 4-slot sponsor placement branches.</p>
  </div>
  <div class="d-flex gap-2 flex-wrap">
    <a href="{{ route('admin.sponsor-pool.index') }}" class="btn btn-outline-secondary rounded-pill px-4">
...
<div class="row g-4 mb-4">
  <div class="col-lg-5">
    <div class="admin-card h-100">
      ...
      <form method="GET" action="{{ route('admin.sponsor-pool.tree') }}" class="row g-3">
        <div class="col-12">
          <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Search sponsor, purchaser, member ID, mobile or node ID">
        </div>
        <div class="col-12 d-flex gap-2">
          <button class="btn btn-main flex-fill"><i class="fa fa-search me-1"></i> Search</button>
          <a href="{{ route('admin.sponsor-pool.tree') }}" class="btn btn-secondary rounded-pill px-4">Reset</a>
        </div>
      </form>
        ...
        <div class="mt-4">
          ...
          @forelse($searchNodes as $result)
            ...
            <div class="tree-result">
              ...
              <a href="{{ route('admin.sponsor-pool.tree', ['node' => $result->id]) }}" class="btn btn-sm btn-outline-dark">Focus</a>
            </div>
          @empty
...
<div class="tree-stage">
  @if($tree)
    <div class="tree-canvas">
      <div class="tree">
        @include('admin.sponsor-pool.partials.tree-node', ['item' => $tree])
      </div>
    </div>
  @else
    <div class="text-center text-muted py-5">No Sponsor pool root found yet.</div>
  @endif
</div>
@endsection

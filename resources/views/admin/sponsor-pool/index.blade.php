...
@section('title', 'Sponsor Pool')
@section('page-title', 'Sponsor Pool Member View')
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-4">
  <div>
    <h4 class="fw-bold mb-1">Sponsor Pool Listing</h4>
    <p class="text-muted mb-0">Sponsor placement records and completed pool payouts.</p>
  </div>
  <a href="{{ route('admin.sponsor-pool.tree') }}" class="btn btn-main rounded-pill px-4">
    <i class="fa fa-sitemap me-1"></i> Tree View
  </a>
</div>

<div class="row g-4 mb-4">
  <div class="col-lg-3 col-md-6"><div class="admin-card pool-stat"><p>Total Pool Nodes</p><h3>{{ number_format($totalNodes) }}</h3></div></div>
  <div class="col-lg-3 col-md-6"><div class="admin-card pool-stat"><p>Sponsor Members</p><h3>{{ number_format($memberNodes) }}</h3></div></div>
  <div class="col-lg-3 col-md-6"><div class="admin-card pool-stat"><p>Completed Level Payouts</p><h3>{{ number_format($paidLevels) }}</h3></div></div>
  <div class="col-lg-3 col-md-6"><div class="admin-card pool-stat"><p>Total Pool Paid</p><h3>{{ $money($totalPaid) }}</h3></div></div>
</div>
    ...
    <div class="admin-card h-100">
      ...
      @forelse($depthStats as $stat)
        ...
        <div class="mb-3">
          ...
          <div class="progress"><div class="progress-bar bg-dark" style="width: {{ $percent }}%"></div></div>
        </div>
      @empty
...
<div class="admin-card mb-4">
  <form method="GET" action="{{ route('admin.sponsor-pool.index') }}">
    <div class="row g-3 align-items-end">
      <div class="col-md-5">
        <label class="form-label">Search</label>
        <input type="text" name="search" value="{{ request('search') }}" class="form-control" placeholder="Pool ID, sponsor, purchaser, member ID, mobile or email">
      </div>
      <div class="col-md-2">
      ...
      <div class="col-md-3 d-flex gap-2">
        ...
        <a href="{{ route('admin.sponsor-pool.index') }}" class="btn btn-secondary rounded-pill px-4">Reset</a>
      </div>
    </div>
  </form>
</div>
...
<div class="admin-card mb-4">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    <h5 class="fw-bold mb-0">Sponsor Pool Placement</h5>
      ...
      <tbody>
        @forelse($nodes as $node)
          ...
          <tr>
            ...
            <td>
              <div class="pool-node">
                <span class="node-avatar {{ $node->admin_id ? 'admin' : '' }}">{{ $node->admin_id ? 'A' : strtoupper(substr($owner?->name ?? 'S', 0, 1)) }}</span>
                <div>
            ...
            <td>
              {{ $node->purchaser?->name ?? '-' }}<br>
              <small class="text-muted">{{ $node->purchaser?->member_id ?? '-' }}</small>
            </td>
            <td>
              {{ $parentOwner?->name ?? '-' }}<br>
        ...
        @empty
          <tr><td colspan="10" class="text-center text-muted py-4">No Sponsor pool members found.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
...
<div class="admin-card">
  <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    <h5 class="fw-bold mb-0">Recent Completed Sponsor Pool Incomes</h5>
  ...
  <div class="table-responsive">
    <table class="table table-bordered align-middle">
      ...
      <tbody>
        ...
        @empty
          <tr><td colspan="6" class="text-center text-muted py-4">No completed Sponsor pool incomes yet.</td></tr>
        @endforelse
      </tbody>
    </table>
  </div>
